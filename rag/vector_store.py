...
class VectorStore:
    ...
    def query_batch(self, zs: np.ndarray, k: int = 10):
        """
        Batch query — retrieve k nearest neighbors for MULTIPLE query vectors at once.
        This is the key speedup: one FAISS call instead of N individual calls.

        Args:
            zs: 2-D array shape (M, dim) — M query vectors.
            k:  Number of neighbors per query.

        Returns:
            List of M lists, each containing k dicts {'label': int, 'dist': float}.
        ...
        zs_np    = _to_numpy(zs).reshape(-1, self.dim).astype("float32")
        M        = zs_np.shape[0]
        k_actual = min(k, n_stored)

        # Single FAISS call for all M queries — this is the entire speedup
        distances, indices = self.index.search(zs_np, k_actual)
