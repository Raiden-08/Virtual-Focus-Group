...
class LSTMEncoder(nn.Module):
    ...
    def __init__(self, d_in: int = 1, hidden_size: int = 64,
        ...
        # Dual Heads:
        # 1. Reconstruction head: t (last element of x_window)
        self.recon_head = nn.Linear(hidden_size, d_in)
        # 2. Prediction head: t+1 (next forecast step)
    ...
    def forward(self, x_window: torch.Tensor):
        ...
        Returns:
            ...
            x_recon : Tensor [B, d_in]         reconstruction of t
            x_pred  : Tensor [B, d_in]         prediction of t+1
        """
        ...
        x_recon = self.recon_head(h)     # [B, d_in]
        x_pred  = self.pred_head(h)      # [B, d_in]

        return h, x_recon, x_pred
